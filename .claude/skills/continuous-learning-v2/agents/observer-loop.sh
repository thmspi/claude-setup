...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${CUSTOM_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set CUSTOM_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
...
PROMPT
  ...
  timeout_seconds="${CUSTOM_OBSERVER_TIMEOUT_SECONDS:-120}"
  max_turns="${CUSTOM_OBSERVER_MAX_TURNS:-10}"
  exit_code=0
  ...
  CUSTOM_SKIP_OBSERVE=1 CUSTOM_HOOK_PROFILE=minimal claude --model haiku --max-turns "$max_turns" --print < "$prompt_file" >> "$LOG_FILE" 2>&1 &
  claude_pid=$!
